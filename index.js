...
dialog.addEventListener("toggle", (e) => {
  if (e.newState === "open") return (overlay.style.display = "block");
...
submitFormButton.addEventListener("click", (e) => {
  ...
  const formChildren = addBookForm.children;
  for (const child of Array.from(formChildren).filter(
  ...
  const [bookName, author, pages] = Array.from(formElements).map(
    (item) => item.value
  );
